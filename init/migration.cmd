...
REM Read agent-core version (used when refreshing entrypoint stamp).
set VERSION=unknown
...
:check_entrypoint
  ...
  if errorlevel 1 (
    ...
    if errorlevel 1 (
      echo.
      echo [Step 5] %_file% has no Agent Core marker — assumed user-authored
...
:write_entrypoint
  ...
  REM "scaffold + blank + --- + Generated by agent-core vX.Y.Z" footer.
  copy /y "%~1" "%~2" >nul
  echo.>> "%~2"
  echo --- >> "%~2"
  echo Generated by agent-core v%VERSION% >> "%~2"
  exit /b 0
